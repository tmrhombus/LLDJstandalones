...
void plotter_L1PF(TString region, TString varname, Bool_t dolog)
{
 ...
 file_nominal      = TFile::Open( inpath + region + "_AllJets_AODCaloJet"+varname+"_GH.root"  ) ; 
 h_nominal         = (TH1F*)file_nominal->Get("bkgtotal")->Clone( "Original" ) ;

 file_L1PF      = TFile::Open( inpath + region + "_AllJets_AODCaloJet_L1PF"+varname+"_GH.root"  ) ; 
 ...
 h_nominal->SetMaximum(h_nominal->GetMaximum()*1.50); 
 TLegend *leg;
 ...
 ratiopad->cd();
       ...
       h_rat->GetYaxis()->SetTitleSize(40);
       ...
       h_rat->GetYaxis()->SetTitle("Orig-Test/Orig");
       ...
       h_rat->GetYaxis()->SetRangeUser(-0.3,0.3);
       h_rat->Draw("p");  // draw first to get ranges set internally inside root
       ...
       line->Draw();
       h_rat->Draw("p same"); // draw points above line
