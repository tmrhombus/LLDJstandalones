...
printf "process.MessageLogger.cerr.threshold = 'ERROR' \n" >> "${thesubdir}/run_data_80X.py"  
printf "process.MessageLogger.cerr.threshold = 'ERROR' \n" >> "${thesubdir}/run_mc_80X.py"              

printf "process.MessageLogger.cerr.FwkReport.reportEvery = 1000000 \n" >> "${thesubdir}/run_data_80X.py"  
printf "process.MessageLogger.cerr.FwkReport.reportEvery = 1000000 \n" >> "${thesubdir}/run_mc_80X.py"              

printf 'process.TFileService.fileName=cms.string("$outputFileName") \n' >> "${thesubdir}/run_data_80X.py"  
printf 'process.TFileService.fileName=cms.string("$outputFileName") \n' >> "${thesubdir}/run_mc_80X.py"              

printf 'process.source = cms.Source("PoolSource", fileNames = cms.untracked.vstring("$inputFileNames") \n' >> "${thesubdir}/run_data_80X.py"  
printf 'process.source = cms.Source("PoolSource", fileNames = cms.untracked.vstring("$inputFileNames") \n' >> "${thesubdir}/run_mc_80X.py"  
...
do
 ...
 submitfile="${thesubdir}/${submitname}.sh"
 ...
 UPERJOB="5"
 SPLITTING="'FileBased'"
 REQUESTNAME="${nversion}_${samplename}"
 DATASET=${datasetname}
 ...
 cp ${subdir}/faj_template.sh              "${submitfile}"
